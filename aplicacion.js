(function () {
  ...
  function translateError(error) {
    ...
    const messages = {
      ...
      OPERACION_NO_ACTIVA:'La operación ya no está activa.', MOTIVO_CORRECCION_OPERACION_REQUERIDO:'Escriba un motivo de al menos 10 caracteres para cerrar administrativamente el registro.', RUTA_ACTIVA_REQUIERE_CIERRE:'Esta operación está vinculada a una ruta que continúa En curso. Finalice o cancele primero la ruta.', CORREO_YA_EXISTE:'El correo ya está registrado.', DIRECCION_APLICACION_NO_CONFIGURADA:'Falta configurar la dirección de la aplicación en configuracion.js.',
  ...
  function precargarModulos() {
    // Regla 4.2.39: no consultar todos los módulos a la vez. Cada módulo
  ...
  const renderers = {
    async dashboard() {
      ...
      const headingActions=`<button class="btn soft" data-sync>↻ Actualizar</button>${hasPermission('RUTAS','CREAR')?'<button class="btn primary" data-new-route>＋ Asignar ruta</button>':''}`;
  ...
  async function renderOperations() {
    ...
    // Compatibilidad con respuestas nuevas y heredadas de flotas-api.
    // Una operación Activa con FECHA_FIN vacía siempre debe aparecer en este módulo.
    const operationRows=Array.isArray(summary.operations)?summary.operations:(Array.isArray(summary.rows)?summary.rows:[]);
    const active=Array.isArray(summary.activeOperations)?summary.activeOperations:operationRows.filter(row=>row.ESTADO==='Activa'&&!row.FECHA_FIN);
  ...
  async function renderGps() {
    ...
    return heading('MONITOREO','GPS en tiempo real','Posición confiable, dirección, velocidad y conexión de la flota autorizada.',controls)+
  ...
  function asegurarComponenteMapa(){
    ...
    promesaComponenteMapa=new Promise((resolve,reject)=>{
      ...
      script.src='mapa.js?v=4.2.39-ui1';
      script.async=true;
  ...
  function bindSection() {
    ...
    $$('[data-close-orphan-operation]').forEach(btn=>btn.addEventListener('click',()=>openCloseOrphanOperationModal(btn.dataset.closeOrphanOperation)));
  ...
  function openCloseOrphanOperationModal(id){
    if(currentUser?.ROL_ID!=='ROL-ADMIN'||!hasPermission('OPERACIONES','EDITAR_ADMIN'))return toast('Acceso restringido','Solo un Administrador autorizado puede cerrar un registro inconsistente.','error');
    const operation=registroFormulario('operations',id)||(cacheListasFormulario.get('operations')||[]).find(row=>String(row.ID)===String(id));
    if(!operation)return toast('Operación no encontrada','Actualice el módulo e intente nuevamente.','error');
    if(operation.RUTA_ID)return toast('Ruta activa vinculada','Finalice o cancele primero la ruta relacionada.','warning');
    $('#modalEyebrow').textContent='CORRECCIÓN ADMINISTRATIVA';$('#modalTitle').textContent=`Cerrar registro ${esc(operation.ID)}`;
    $('#modalBody').innerHTML=`<form class="form-grid" id="closeOrphanOperationForm"><div class="module-diagnostic warning full"><i>!</i><div><b>Use esta opción solo si la operación ya terminó y quedó abierta por una inconsistencia</b><span>No requiere GPS de cierre. Se registrarán Administrador, fecha, IP, motivo, historial y auditoría. El conductor y vehículo se liberarán únicamente si no tienen otra operación o ruta activa.</span></div></div><label class="field full"><span>Motivo obligatorio</span><textarea name="MOTIVO_CORRECCION" minlength="10" required placeholder="Ejemplo: Operación terminada previamente; el cierre no quedó registrado por pérdida de conexión."></textarea></label><div class="form-actions"><button class="btn soft" type="button" data-cancel-modal>Cancelar</button><button class="btn danger" type="submit">Cerrar y liberar si corresponde</button></div></form>`;
    openModal();const form=$('#closeOrphanOperationForm');$('[data-cancel-modal]',form).onclick=closeModal;form.onsubmit=async event=>{event.preventDefault();const button=$('button[type="submit"]',form);await conCargaBoton(button,'Cerrando registro…',async()=>{try{const data=Object.fromEntries(new FormData(form).entries());data.IP_PUBLICA=clientPublicIp||await api.getClientIp?.().catch(()=> '')||'';await api.request('closeOrphanOperationAdmin',{id,data});invalidarListasFormulario('operations','vehicles','drivers','history','audit');['operations','dashboard','history','audit'].forEach(section=>cacheVistasModulo.delete(section));closeModal();toast('Registro corregido','La operación fue cerrada administrativamente y la disponibilidad fue recalculada.','success');actualizarSeccionEnSegundoPlano('operations');}catch(error){toast('No se pudo corregir',translateError(error),'error');}});};
  }
