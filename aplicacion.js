(function () {
  ...
  function hasPermission(module,action='LEER'){
    ...
    const driverOwnGps=String(currentUser?.ROL_ID||'').toUpperCase()==='ROL-CONDUCTOR'&&module==='GPS'&&action==='LEER';
    return driverOwnGps||esAdministrador()||permissions.includes('*:*')||permissions.includes(`${module}:${action}`);
  }
  ...
  function puedeReenviarAlertaAsignacion(){return ['ROL-ADMIN','ROL-SUPERVISOR'].includes(String(currentUser?.ROL_ID||'').toUpperCase());}
  ...
  function avisosEmergentesActivos(){
    if(!['ROL-ADMIN','ROL-SUPERVISOR'].includes(String(currentUser?.ROL_ID||'').toUpperCase()))return true;
  ...
  function translateError(error) {
    ...
    const messages = {
      ...
      ALERTA_OPERACIONAL_REQUIERE_ADMINISTRADOR:'Esta alerta operacional debe ser validada y cerrada por un Administrador real después de comprobar la situación en terreno.', ALERTA_ASIGNACION_NO_AUTORIZADA:'Solo Administradores y Supervisores pueden enviar o reenviar alertas emergentes de asignación.', ASIGNACION_REENVIO_INVALIDA:'No fue posible identificar la ruta u operación que desea reenviar.', ASIGNACION_REQUIERE_ACEPTACION:'Esta asignación permanecerá pendiente hasta que el conductor presione Aceptar.',
  ...
  function cargarListaFormulario(resource) {
    ...
    const pending = api.request('list',{resource,limit:1000})
  ...
  function esAvisoAsignacion(item){return ['RUTA_ASIGNADA','OPERACION_ASIGNADA'].includes(String(item?.CATEGORIA_EMERGENTE||'').toUpperCase())&&String(item?.ESTADO_RESPUESTA||'PENDIENTE').toUpperCase()==='PENDIENTE';}
  ...
  function showAssignmentAlert(item){
    ...
    assignmentAlertNode?.remove();const clase=String(item.CATEGORIA_EMERGENTE||'').startsWith('OPERACION')?'operación':'ruta';
    const node=document.createElement('section');node.className='assignment-alert-card';node.setAttribute('role','alertdialog');
    const etiquetaAceptar=esAdministrador()?'Aceptar como Administrador':puedeAceptarAsignacionesAjenas()?'Aceptar como Supervisor':'Aceptar';
    node.innerHTML=`<header><i>${clase==='ruta'?'➜':'⇄'}</i><div><span>AVISO PRIORITARIO</span><h2>Nueva ${clase} asignada</h2></div><button type="button" data-assignment-response="CERRADA" aria-label="Cerrar">×</button></header><div class="assignment-alert-body"><h3>${esc(item.NOMBRE_ASIGNACION||item.TITULO||'Nueva asignación')}</h3><div><span>Usuario</span><b>${esc(item.DESTINATARIO_NOMBRE||currentUser?.NOMBRE||'Conductor')}</b></div><div><span>Desde</span><b>${esc(item.ORIGEN||'No informado')}</b></div><div><span>Hasta</span><b>${esc(item.DESTINO||'No informado')}</b></div><p>${item.DISTANCIA_KM==null?'Distancia por calcular':`${esc(item.DISTANCIA_KM)} km estimados`} · ${item.DURACION_MINUTOS==null?'Tiempo por calcular':`${esc(item.DURACION_MINUTOS)} min estimados`}</p></div><footer><button class="danger" data-assignment-response="CERRADA">× Cerrar</button><button class="primary" data-assignment-response="ACEPTADA">✓ ${etiquetaAceptar}</button></footer>`;
    ...
    if(vozAsignacionesActiva())hablar(`Nueva ${clase} asignada para ${item.DESTINATARIO_NOMBRE||'el conductor'}. ${item.NOMBRE_ASIGNACION||''}. Desde ${item.ORIGEN||'origen no informado'} hasta ${item.DESTINO||'destino no informado'}.`);
    setTimeout(()=>{if(assignmentAlertNode===node&&node.dataset.persistente!=='1'&&!document.hidden){node.remove();assignmentAlertNode=null;}},6000);
  }
  ...
  function notificationCard(item){
    ...
    const etiquetaAceptacion=esAdministrador()?'Aceptar como Administrador':puedeAceptarAsignacionesAjenas()?'Aceptar como Supervisor autorizado':'Aceptar asignación';
  ...
  async function renderNotifications() {
    ...
    const rows=notifications.map(item=>{const recipient=driverMap[item.DESTINATARIO_CONDUCTOR_ID]?.NOMBRE||userMap[item.DESTINATARIO_USUARIO_ID]?.NOMBRE||item.DESTINATARIO_CONDUCTOR_ID||item.DESTINATARIO_USUARIO_ID||'Sin destinatario';const etiqueta=esAdministrador()?'Aceptar como Administrador':puedeAceptarAsignacionesAjenas()?'Aceptar como Supervisor':'Aceptar';const accion=item.LEIDA==='SI'?'—':esAvisoAsignacion(item)?`<button class="btn primary small" data-accept-assignment="${item.ID}">✓ ${etiqueta}</button>`:`<button class="btn soft small" data-read-notification="${item.ID}">Marcar leída</button>`;return `<tr data-filter-date="${esc(item.FECHA_ENVIO||item.CREADO_EN||'')}" data-search-row="${esc(`${item.TITULO} ${item.MENSAJE} ${recipient} ${item.PRIORIDAD} ${item.TIPO}`.toLowerCase())}"><td>${item.LEIDA==='SI'?'<span class="status">Leída</span>':'<span class="status warning">Pendiente</span>'}</td><td><strong>${esc(item.TITULO)}</strong><span class="muted">${esc(item.MENSAJE)}</span></td><td>${esc(recipient)}</td><td>${status(item.PRIORIDAD||'Normal')}</td><td>${esc(item.TIPO||'Información')}</td><td>${fmtDate(item.FECHA_ENVIO||item.CREADO_EN,true)}</td><td>${accion}</td></tr>`;}).join('');
  ...
  function openOperationModal(prefillVehicle=null) {
    ...
    const controlAlerta=['ROL-ADMIN','ROL-SUPERVISOR'].includes(String(currentUser?.ROL_ID||'').toUpperCase())?`<label class="assignment-alert-switch full"><input type="checkbox" name="ENVIAR_ALERTA_ASIGNACION" value="SI" checked><i></i><span><b>Enviar alerta emergente</b><small>Solo Administrador o Supervisor puede omitirla; la voz depende de la preferencia del conductor.</small></span></label>`:'';
