(function () {
  ...
  const navGroups = [
    ['GENERAL', [
      ['dashboard','⌂','Panel principal'], ['office','◆','Oficina Virtual'], ['routes','➜','Rutas asignadas'], ['checkin','✓','Check-in vehicular'], ['operations','⇄','Operaciones'], ['gps','⌖','Ubicación en tiempo real'],
  ...
  const labels = {
    dashboard:'Panel principal',office:'Oficina Virtual',routes:'Rutas asignadas',vehicles:'Vehículos',drivers:'Conductores',checkin:'Check-in vehicular',checkins:'Check-ins',checkinApprovals:'Aprobación de check-ins',checkinHistory:'Historial de check-in',operations:'Operaciones',gps:'Ubicación en tiempo real',maintenance:'Mantenciones',fuel:'Combustible',
    notifications:'Notificaciones',documents:'Documentos del conductor',history:'Historial',alerts:'Alertas',connections:'Conexiones en línea',users:'Usuarios',reports:'Reportes',audit:'Auditoría',company:'Empresa',settings:'Configuración'
  };
  ...
  function esAvisoAsignacion(item){return ['RUTA_ASIGNADA','OPERACION_ASIGNADA','VEHICULO_CHECKIN_ASIGNADO'].includes(String(item?.CATEGORIA_EMERGENTE||'').toUpperCase())&&String(item?.ESTADO_RESPUESTA||'PENDIENTE').toUpperCase()==='PENDIENTE';}
  async function responderAvisoAsignacionWeb(item,respuesta,button){
    if(!item?.ID)return;if(button)button.disabled=true;
    try{await api.request('respondAssignmentAlert',{id:item.ID,data:{NOTIFICACION_ID:item.ID,RESPUESTA:respuesta}});assignmentAlertNode?.remove();assignmentAlertNode=null;knownNotificationIds.delete(String(item.ID));invalidarListasFormulario('notifications','routes','operations');await refreshNotificationBadge();toast(respuesta==='ACEPTADA'?'Asignación aceptada':'Aviso cerrado','La respuesta quedó confirmada en el sistema.');}
  ...
  function showAssignmentAlert(item){
    ...
    assignmentAlertNode?.remove();const categoria=String(item.CATEGORIA_EMERGENTE||'').toUpperCase(),clase=categoria.startsWith('VEHICULO_CHECKIN')?'vehículo para check-in':categoria.startsWith('OPERACION')?'operación':'ruta',encabezado=categoria.startsWith('VEHICULO_CHECKIN')?'Nuevo vehículo asignado':`Nueva ${clase} asignada`;
    const node=document.createElement('section');node.className='assignment-alert-card';node.setAttribute('role','alertdialog');
    const etiquetaAceptar=esAdministrador()?'Aceptar como Administrador':puedeAceptarAsignacionesAjenas()?'Aceptar como Operador':'Aceptar';
    node.innerHTML=`<header><i>${clase==='ruta'?'➜':clase==='operación'?'⇄':'▣'}</i><div><span>AVISO PRIORITARIO</span><h2>${encabezado}</h2></div><button type="button" data-assignment-response="CERRADA" aria-label="Cerrar">×</button></header><div class="assignment-alert-body"><h3>${esc(item.NOMBRE_ASIGNACION||item.TITULO||'Nueva asignación')}</h3><div><span>Usuario</span><b>${esc(item.DESTINATARIO_NOMBRE||currentUser?.NOMBRE||'Conductor')}</b></div><div><span>Desde</span><b>${esc(item.ORIGEN||'No informado')}</b></div><div><span>Hasta</span><b>${esc(item.DESTINO||'No informado')}</b></div><p>${categoria.startsWith('VEHICULO_CHECKIN')?'Retire la llave y complete el check-in antes de iniciar la ruta.':`${item.DISTANCIA_KM==null?'Distancia por calcular':`${esc(item.DISTANCIA_KM)} km estimados`} · ${item.DURACION_MINUTOS==null?'Tiempo por calcular':`${esc(item.DURACION_MINUTOS)} min estimados`}`}</p></div><footer><button class="danger" data-assignment-response="CERRADA">× Cerrar</button><button class="primary" data-assignment-response="ACEPTADA">✓ ${etiquetaAceptar}</button></footer>`;
    document.body.append(node);assignmentAlertNode=node;$$('[data-assignment-response]',node).forEach(button=>button.onclick=()=>responderAvisoAsignacionWeb(item,button.dataset.assignmentResponse,button));
  ...
  function precargarModulos() {
    // Regla 4.2.46: no consultar todos los módulos a la vez. Cada módulo
  ...
  function routeTimelineMarkup(route){if(!puedeVerLineaTiempoRuta())return'';const events=Array.isArray(route?.LINEA_TIEMPO)?route.LINEA_TIEMPO:[];if(!events.length)return'<div class="route-timeline empty"><span>Línea de tiempo pendiente de eventos</span></div>';let previous=0;return`<div class="route-timeline">${events.map(event=>{const current=new Date(event.FECHA_EVENTO||event.CREADO_EN||0).getTime(),seconds=Number(event.DURACION_DESDE_ANTERIOR_SEGUNDOS||((previous&&current)?Math.max(0,Math.round((current-previous)/1000)):0));previous=current||previous;return`<div><i></i><span><b>${esc(String(event.EVENTO||'Evento').replaceAll('_',' '))}</b><small>${fmtDate(event.FECHA_EVENTO||event.CREADO_EN,true)}${seconds?` · etapa ${formatRouteElapsed(seconds)}`:''}</small>${event.USUARIO_NOMBRE?`<em>${esc(event.USUARIO_NOMBRE)}</em>`:''}</span></div>`;}).join('')}</div>`;}
  ...
  function notificationCard(item){
    ...
    const accion=item.LEIDA==='SI'?'':esAvisoAsignacion(item)?`<button class="link-button assignment-accept-button" data-accept-assignment="${item.ID}" type="button">✓ ${etiquetaAceptacion}</button>`:`<button class="link-button" data-read-notification="${item.ID}" type="button">Marcar como leída</button>`;
  ...
  function checkinRow(row,vehicleMap,driverMap,withReview=false) {
    ...
    const review=withReview&&['Pendiente','Bloqueado'].includes(row.ESTADO_REVISION)?`<button class="btn primary small" data-review-checkin="${esc(row.ID)}">Revisar y autorizar</button>`:'';
  ...
  function checkinInlineFormMarkup() {
    ...
    const selectedVehicle=assignedVehicle?.ID||pendienteRuta.VEHICULO_ID||'',selectedDriver=driverProfile?assignedDriverId:(pendienteRuta.CONDUCTOR_ID||assignedDriverId);
  ...
  async function renderCheckin() {
    ...
    const create=`${hasPermission('CHECKIN','ASIGNAR_VEHICULO')?'<button class="btn primary" data-assign-checkin-vehicle>＋ Asignar vehículo y alertar</button>':''}${hasPermission('CHECKIN','CREAR')?`${hasPermission('CHECKIN','VALIDAR_QR')?'<button class="btn soft" data-open-checkin-qr>▦ Escanear QR para revisión</button>':''}<button class="btn primary" data-focus-checkin>↓ Ir a la lista de chequeo</button>`:''}`;
    return heading('INSPECCIÓN PREOPERACIONAL','Check-in vehicular','Revise el vehículo antes de iniciar cualquier operación. Los 16 controles aparecen directamente en esta pantalla.',`<button class="btn soft" data-sync>↻ Actualizar</button>${create}`)+
      reciboCheckinMarkup()+
      `<div class="checkin-process"><article><i>1</i><div><b>Vehículo asignado</b><span>Confirme patente, conductor y kilometraje.</span></div></article><article><i>2</i><div><b>Completar ${checkinCatalog.length} controles</b><span>Marque Conforme o No conforme en cada punto.</span></div></article><article><i>3</i><div><b>Finalizar check-in</b><span>Administrador, Gerencia y Operador recibirán el informe.</span></div></article></div>`+
  ...
  async function renderCheckinApprovals() {
    ...
    return heading('CONTROL DE SEGURIDAD','Aprobación de check-ins','Revise observaciones leves. Las fallas críticas exigen corrección y una nueva inspección.',`<button class="btn soft" data-sync>↻ Actualizar</button>`)+
      `<div class="operation-banner checkin-warning"><i>!</i><div><h3>Regla de bloqueo</h3><p>Un operador puede aprobar observaciones leves, pero nunca una inspección con fallas críticas.</p></div></div>`+
  ...
  async function renderRoutes() {
    ...
    return heading('PLANIFICACIÓN OPERACIONAL','Asignación de rutas','Cree, supervise y cierre rutas vinculadas al conductor, vehículo y punto base.',actions)+
      ...
      `<article class="card"><div class="card-header"><div><h3>Historial de rutas</h3><p>Todos los estados y destinos registrados</p></div></div><div class="toolbar route-filter-toolbar"><label class="search-box"><span>⌕</span><input data-table-search placeholder="Buscar ruta, conductor, vehículo o destino"></label>${selectorEstadosRuta(routes)}${puedeExportarFormato('csv')?'<button class="btn soft push" data-export="routes">Exportar CSV</button>':''}</div><div data-filter-table>${table(['Ruta','Conductor','Vehículo','Recorrido','Asignada','Tiempo','Estado','Evidencias','Acciones'],routeRows,'No existen rutas registradas.')}</div></article>`;
  }
  ...
  async function renderNotifications() {
    ...
    const rows=notifications.map(item=>{const recipient=driverMap[item.DESTINATARIO_CONDUCTOR_ID]?.NOMBRE||userMap[item.DESTINATARIO_USUARIO_ID]?.NOMBRE||item.DESTINATARIO_CONDUCTOR_ID||item.DESTINATARIO_USUARIO_ID||'Sin destinatario';const etiqueta=esAdministrador()?'Aceptar como Administrador':puedeAceptarAsignacionesAjenas()?'Aceptar como Operador':'Aceptar';const accion=item.LEIDA==='SI'?'—':esAvisoAsignacion(item)?`<button class="btn primary small" data-accept-assignment="${item.ID}">✓ ${etiqueta}</button>`:`<button class="btn soft small" data-read-notification="${item.ID}">Marcar leída</button>`;return `<tr data-filter-date="${esc(item.FECHA_ENVIO||item.CREADO_EN||'')}" data-search-row="${esc(`${item.TITULO} ${item.MENSAJE} ${recipient} ${item.PRIORIDAD} ${item.TIPO}`.toLowerCase())}"><td>${item.LEIDA==='SI'?'<span class="status">Leída</span>':'<span class="status warning">Pendiente</span>'}</td><td><strong>${esc(item.TITULO)}</strong><span class="muted">${esc(item.MENSAJE)}</span></td><td>${esc(recipient)}</td><td>${status(item.PRIORIDAD||'Normal')}</td><td>${esc(item.TIPO||'Información')}</td><td>${fmtDate(item.FECHA_ENVIO||item.CREADO_EN,true)}</td><td>${accion}</td></tr>`;}).join('');
  ...
  function asegurarComponenteMapa(){
    ...
    promesaComponenteMapa=new Promise((resolve,reject)=>{
      ...
      script.src='mapa.js?v=4.2.46-ui5';
      script.async=true;
  ...
  async function pintarKpisReportes(recargar=false){
    ...
    const vehicleKpis=Array.isArray(data.KPI_VEHICULOS)?data.KPI_VEHICULOS:[];
  ...
  async function submitInlineCheckin(form) {
    ...
    await conCargaBoton(button,'Guardando en la base…',async()=>{try{
      ...
      toast(state==='Bloqueado'?'Salida bloqueada':'Check-in guardado y visible',`${isCentral?'Base central confirmada':'Almacenamiento local'} · ${confirmado.ID}. ${state==='Aprobado'?'La inspección quedó aprobada y vigente durante el día para este vehículo y conductor.':state==='Pendiente'?'Un operador debe revisar las observaciones antes de iniciar.':'Se detectaron fallas críticas.'}`,state==='Bloqueado'?'error':'success');
  ...
  function aplicarVehiculoQrCheckin(vehicle) {
    if(String(currentUser?.ROL_ID||'').toUpperCase()==='ROL-CONDUCTOR'){
      const asignado=listaFormulario('vehicles')[0];
      if(!asignado||String(asignado.ID)!==String(vehicle?.ID||'')){toast('Vehículo no asignado','El conductor solo puede realizar el check-in del vehículo que le asignó la empresa.','error');return;}
    }
  ...
  function openCheckinReviewModal(id) {
    ...
    $('#modalBody').innerHTML=`<div class="checkin-review">${checkinDetailMarkup(row).replace(/<div class="form-actions">[\s\S]*?<\/div><\/div>$/,'')}</div>${noConforme?'<div class="modal-error full"><b>Autorización excepcional de conducción</b><p>Al aprobar, el conductor podrá iniciar mientras este check-in esté vigente. El nombre, rol, fecha y comentario del autorizador quedarán auditados.</p></div>':''}<label class="field"><span>Comentario del autorizador</span><textarea id="checkinReviewComment" required placeholder="Indique la decisión, medidas y condiciones de conducción"></textarea></label><div class="form-actions"><button class="btn soft" type="button" data-cancel-modal>Cancelar</button><button class="btn danger" type="button" data-checkin-decision="RECHAZAR">Rechazar</button><button class="btn primary" type="button" data-checkin-decision="APROBAR">${noConforme?'Autorizar conducción':'Aprobar check-in'}</button></div></div>`;
