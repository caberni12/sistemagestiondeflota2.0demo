(function () {
  ...
  async function parseXlsxFile(file,maxRows=1500){
    ...
    if(workbookFile&&relsFile){const workbookXml=new DOMParser().parseFromString(await workbookFile.async('string'),'application/xml'),properties=workbookXml.getElementsByTagName('workbookPr')[0];date1904=String(properties?.getAttribute('date1904')||'').toLowerCase()==='true'||properties?.getAttribute('date1904')==='1';const sheets=[...workbookXml.getElementsByTagName('sheet')],selected=sheets.find(node=>normalizeImportHeader(node.getAttribute('name'))==='DATOS')||sheets[0],relId=selected?.getAttribute('r:id')||selected?.getAttributeNS('http://schemas.openxmlformats.org/officeDocument/2006/relationships','id'),relsXml=new DOMParser().parseFromString(await relsFile.async('string'),'application/xml'),rel=[...relsXml.getElementsByTagName('Relationship')].find(node=>node.getAttribute('Id')===relId),target=rel?.getAttribute('Target');if(target)sheetPath='xl/'+target.replace(/^\//,'').replace(/^xl\//,'').replace(/^\.\.\//,'');}
