(function () {
  ...
  async function solicitarListaPaginada(resource,{limit=limiteRegistrosActual(),cache=false}={}){
    ...
    while(true){
      ...
      if(page.length<1000||last.hasMore===false||added===0)break;
      offset+=1000;
    }
  ...
  function precargarModulos() {
    // Regla 4.2.42: no consultar todos los módulos a la vez. Cada módulo
  ...
  function asegurarComponenteMapa(){
    ...
    promesaComponenteMapa=new Promise((resolve,reject)=>{
      ...
      script.src='mapa.js?v=4.2.42-ui2';
      script.async=true;
  ...
  async function exportarKpisFiltrados(formato='csv'){
    ...
    await exporter.exportarFilas(operations,{formato,nombre:'KPI_Operaciones',titulo:'Reporte de operaciones filtradas',subtitulo:'Indicadores operacionales del Sistema de Gestión de Flotas',autor:currentUser?.NOMBRE||currentUser?.CORREO||'',hoja:'Operaciones',metadatos:{'Fecha desde':data.desde?fmtDate(data.desde,false):'Sin límite','Fecha hasta':data.hasta?fmtDate(data.hasta,false):'Sin límite','Conductor':driver?.NOMBRE||'Todos','Vehículo':vehicle?.PATENTE||'Todos','Total operaciones':operations.length}});
