(function () {
  ...
  function showAssignmentAlert(item){
    ...
    hablar(`Nueva ${clase} asignada para ${item.DESTINATARIO_NOMBRE||'el conductor'}. ${item.NOMBRE_ASIGNACION||''}. Desde ${item.ORIGEN||'origen no informado'} hasta ${item.DESTINO||'destino no informado'}.`);
  ...
  async function refreshNotificationBadge(){
    ...
    try{
      ...
      const [notificationResult,assignmentResult,alertResult]=await Promise.all([
        ...
        canNotifications?api.request('assignmentAlerts',{limit:20,cache:false}):Promise.resolve({rows:[]}),
  ...
  function precargarModulos() {
    // Regla 4.2.41: no consultar todos los módulos a la vez. Cada módulo
  ...
  function notificationCard(item){
    ...
    const accion=item.LEIDA==='SI'?'':esAvisoAsignacion(item)?`<button class="link-button assignment-accept-button" data-accept-assignment="${item.ID}" type="button">✓ Aceptar asignación</button>`:`<button class="link-button" data-read-notification="${item.ID}" type="button">Marcar como leída</button>`;
  ...
  async function renderNotifications() {
    ...
    const rows=notifications.map(item=>{const recipient=driverMap[item.DESTINATARIO_CONDUCTOR_ID]?.NOMBRE||userMap[item.DESTINATARIO_USUARIO_ID]?.NOMBRE||item.DESTINATARIO_CONDUCTOR_ID||item.DESTINATARIO_USUARIO_ID||'Sin destinatario';const accion=item.LEIDA==='SI'?'—':esAvisoAsignacion(item)?`<button class="btn primary small" data-accept-assignment="${item.ID}">✓ Aceptar</button>`:`<button class="btn soft small" data-read-notification="${item.ID}">Marcar leída</button>`;return `<tr data-filter-date="${esc(item.FECHA_ENVIO||item.CREADO_EN||'')}" data-search-row="${esc(`${item.TITULO} ${item.MENSAJE} ${recipient} ${item.PRIORIDAD} ${item.TIPO}`.toLowerCase())}"><td>${item.LEIDA==='SI'?'<span class="status">Leída</span>':'<span class="status warning">Pendiente</span>'}</td><td><strong>${esc(item.TITULO)}</strong><span class="muted">${esc(item.MENSAJE)}</span></td><td>${esc(recipient)}</td><td>${status(item.PRIORIDAD||'Normal')}</td><td>${esc(item.TIPO||'Información')}</td><td>${fmtDate(item.FECHA_ENVIO||item.CREADO_EN,true)}</td><td>${accion}</td></tr>`;}).join('');
  ...
  function asegurarComponenteMapa(){
    ...
    promesaComponenteMapa=new Promise((resolve,reject)=>{
      ...
      script.src='mapa.js?v=4.2.41-ui1';
      script.async=true;
  ...
  function openRouteModal(){
    ...
    const controlAlerta=['ROL-ADMIN','ROL-SUPERVISOR'].includes(String(currentUser?.ROL_ID||'').toUpperCase())?`<label class="assignment-alert-switch full"><input type="checkbox" name="ENVIAR_ALERTA_ASIGNACION" value="SI" checked><i></i><span><b>Enviar alerta emergente y de voz</b><small>Activado por defecto. El conductor recibirá este aviso de forma individual.</small></span></label>`:'';
  ...
  function openOperationModal(prefillVehicle=null) {
    ...
    const controlAlerta=['ROL-ADMIN','ROL-SUPERVISOR'].includes(String(currentUser?.ROL_ID||'').toUpperCase())?`<label class="assignment-alert-switch full"><input type="checkbox" name="ENVIAR_ALERTA_ASIGNACION" value="SI" checked><i></i><span><b>Enviar alerta emergente y de voz</b><small>Solo Administrador o Supervisor puede apagarla para esta operación.</small></span></label>`:'';
