(function () {
  ...
  function showAssignmentAlert(item){
    ...
    assignmentAlertNode?.remove();const categoria=String(item.CATEGORIA_EMERGENTE||'').toUpperCase(),clase=categoria.startsWith('VEHICULO_CHECKIN')?'vehículo para check-in':categoria.startsWith('OPERACION')?'operación':'ruta',encabezado=categoria.startsWith('VEHICULO_CHECKIN')?'Vehículo asignado':`Nueva ${clase} asignada`;
  ...
  async function renderRoutes() {
    ...
    const traceRows=sorted.map(route=>{
      ...
      if(route.ESTADO==='En curso'&&hasPermission('RUTAS','COMPLETAR'))actionButtons.push(`<button class="btn primary small" data-route-state="${esc(route.ID)}:Completada">Finalizar</button>`);
