(function () {
  ...
  function markupConversacionOficinaVirtual(){
    return conversacionOficinaVirtual.map(message=>`<div class="office-message ${message.tipo==='usuario'?'user':'assistant'}"><i>${message.tipo==='usuario'?initials(currentUser.NOMBRE):'NX'}</i><div><b>${message.tipo==='usuario'?'Tú':'NEXO IA'}</b><p>${esc(message.texto||'')}</p>${message.tipo==='asistente'?markupDocumentosNexo(message.documentos):''}</div></div>`).join('');
  }
  ...
  async function renderOficinaVirtual(){
    ...
    const suggestions=['Muéstrame mis documentos','Documentos de mi vehículo','¿Qué alertas tengo?','¿Hay excesos de velocidad?','¿Qué tengo pendiente?'];
  ...
  async function enviarConsultaOficinaVirtual(text,button){
    ...
    try{
      ...
      conversacionOficinaVirtual.push({tipo:'asistente',texto:result.respuesta||'No pude generar una respuesta.',documentos:Array.isArray(result.documentos)?result.documentos:[]});
