(function () {
  ...
  const accionesAplicacion = Object.freeze({
    ...
    finishOperation:'finalizarOperacion', closeOrphanOperationAdmin:'cerrarOperacionHuerfanaAdministrativa', editOperationAdmin:'editarOperacionAdministrativa', deleteOperationAdmin:'eliminarOperacionAdministrativa', diagnoseAvailability:'diagnosticarDisponibilidad', saveLocation:'guardarUbicacion', latestLocations:'ultimasUbicaciones',
  ...
  const emptyState = () => ({
    ...
    companies: [], qr: [], routes: [], routeHistory: [], notifications: [], connections: [], checkins: [], fuel: [], fuelAuthorizations: [], sessions: []
  });
  ...
  const accionesLectura = new Set(['status','me','dashboard','operationsSummary','list','realtimeSummary','connectionsOnline','connectionTrackingLive','diagnoseSystem','officeQuickStatus','officeTasks','officeStatus','officeIncidents','getOperationalPoint','fuelSummary','routeEvidenceImage','backupCatalog','backupTable']);
  ...
  function invalidarDespuesDeEscritura(action, payload = {}) {
    ...
    const impacts = {
      ...
      closeOrphanOperationAdmin: { actions:['dashboard','operationsSummary','realtimeSummary'], resources:['operations','vehicles','drivers','routes','history','audit'] },
  ...
  async function localRequest(action, payload) {
    ...
    switch (action) {
      ...
      case 'closeOrphanOperationAdmin': return localCloseOrphanOperationAdmin(payload);
  ...
  function localOperationalUsage(driverId='',vehicleId=''){
    const operations=activeRows(localDb.operations).filter(row=>row.ESTADO==='Activa'&&!row.FECHA_FIN&&(!driverId||row.CONDUCTOR_ID===driverId)&&(!vehicleId||row.VEHICULO_ID===vehicleId));
    const routes=activeRows(localDb.routes).filter(row=>row.ESTADO==='En curso'&&!row.FECHA_FIN&&(!driverId||row.CONDUCTOR_ID===driverId)&&(!vehicleId||row.VEHICULO_ID===vehicleId));
  ...
  async function localCreate(payload) {
    ...
    localDb[key].push(row);let association=null;if(key==='users')association=localAssociateDriverByEmail(row,'CREACION_USUARIO');audit(user,'CREAR',key.toUpperCase(),`Registro creado. Datos: ${JSON.stringify(cleanRow(row))}`,row.ID); saveLocal(); return { row:key==='users'?publicUser(row):cleanRow(row), asociacionConductor:association?{CONDUCTOR_ID:association.driver?.ID||'',ASOCIADO_AUTOMATICAMENTE:association.linked,MOTIVO:association.reason}:undefined };
  }
  ...
  function localValidateFuel(data,existing={},user=null,isCreate=false){
    ...
    if(!admin&&isCreate){if(!operation&&!route)throw new Error('COMBUSTIBLE_ASIGNACION_ACTIVA_REQUERIDA');if(operation&&(operation.ESTADO!=='Activa'||operation.FECHA_FIN))throw new Error('COMBUSTIBLE_ASIGNACION_NO_VIGENTE');if(route&&(!['Asignada','En curso'].includes(route.ESTADO)||route.FECHA_FIN))throw new Error('COMBUSTIBLE_ASIGNACION_NO_VIGENTE');}
  ...
  function localOperationsSummary(payload={}) {
    ...
    const active=ordered.filter(row=>row.ESTADO==='Activa'&&!row.FECHA_FIN);
    const recent=ordered.filter(row=>!(row.ESTADO==='Activa'&&!row.FECHA_FIN)).slice(0,historyLimit);
    const selected=[...new Map([...active,...recent].map(row=>[String(row.ID),row])).values()]
      .sort((a,b)=>a.ESTADO==='Activa'&&!a.FECHA_FIN&&!(b.ESTADO==='Activa'&&!b.FECHA_FIN)?-1:!(a.ESTADO==='Activa'&&!a.FECHA_FIN)&&b.ESTADO==='Activa'&&!b.FECHA_FIN?1:time(b)-time(a));
    ...
    const routes=localFilterRows('routes',activeRows(localDb.routes),user).filter(row=>['Asignada','En curso'].includes(row.ESTADO)&&!row.FECHA_FIN||routeIds.has(String(row.ID))).map(cleanRow);
  ...
  function panelPrincipalLocal() {
    ...
    return { metrics:{ vehicles:rows('vehicles').length,availableVehicles:rows('vehicles').filter(x=>x.ESTADO==='Disponible').length,drivers:rows('drivers').length,
      availableDrivers:rows('drivers').filter(x=>x.ESTADO==='Disponible').length,activeOperations:rows('operations').filter(x=>x.ESTADO==='Activa'&&!x.FECHA_FIN).length,
      ...
      assignedRoutes:rows('routes').filter(x=>['Asignada','En curso'].includes(x.ESTADO)&&!x.FECHA_FIN).length,
  ...
  function localRouteForOperation(data,vehicle,driver,user){if(!data.RUTA_ID)return null;const route=find('routes',data.RUTA_ID);if(!route)throw new Error('RUTA_NO_ENCONTRADA');if(!localFilterRows('routes',[route],user).length)throw new Error('PERMISO_DENEGADO');if(!['Asignada','En curso'].includes(route.ESTADO)||route.FECHA_FIN)throw new Error('RUTA_NO_DISPONIBLE');if(route.CONDUCTOR_ID!==driver.ID)throw new Error('RUTA_NO_COINCIDE_CONDUCTOR');if(route.VEHICULO_ID&&route.VEHICULO_ID!==vehicle.ID)throw new Error('RUTA_NO_COINCIDE_VEHICULO');if(route.OPERACION_ID){const linked=find('operations',route.OPERACION_ID);if(linked?.ESTADO==='Activa'&&!linked.FECHA_FIN)throw new Error('RUTA_YA_VINCULADA');}return route;}
  ...
  function localFinishOperation(payload) {
    ...
    if(!row||row.ESTADO!=='Activa'||row.FECHA_FIN)throw new Error('OPERACION_NO_ACTIVA');if(!localFilterRows('operations',[row],user).length)throw new Error('PERMISO_DENEGADO');
    ...
    const vehicle=find('vehicles',row.VEHICULO_ID),driver=find('drivers',row.CONDUCTOR_ID);if(vehicle&&kmEnd!==''&&(localOptionalKm(vehicle.KILOMETRAJE)===''||kmEnd>=Number(vehicle.KILOMETRAJE||0)))vehicle.KILOMETRAJE=kmEnd;if(row.RUTA_ID){const route=find('routes',row.RUTA_ID);if(route&&['Asignada','En curso'].includes(route.ESTADO)&&!route.FECHA_FIN)Object.assign(route,{ESTADO:'Completada',FECHA_FIN:now,OPERACION_ID:row.ID,ACTUALIZADO_EN:now});}if(vehicle)localReconcileAvailability('vehicles',vehicle);if(driver)localReconcileAvailability('drivers',driver);
  ...
  function localCloseOrphanOperationAdmin(payload){
    const user=requireLocalUser();localRequireOperationAdmin(user);requireLocalPermission(user,'OPERACIONES','EDITAR_ADMIN');
    const data=payload.data||payload,row=find('operations',payload.id||payload.OPERACION_ID||data.OPERACION_ID);
    if(!row)throw new Error('REGISTRO_NO_ENCONTRADO');
    if(row.ESTADO!=='Activa'||row.FECHA_FIN)throw new Error('OPERACION_NO_ACTIVA');
    const reason=String(data.MOTIVO_CORRECCION||data.MOTIVO||'').trim();if(reason.length<10)throw new Error('MOTIVO_CORRECCION_OPERACION_REQUERIDO');
    if(row.RUTA_ID){const route=find('routes',row.RUTA_ID);if(route&&route.ESTADO==='En curso'&&!route.FECHA_FIN)throw new Error('RUTA_ACTIVA_REQUIERE_CIERRE');}
    const now=iso();Object.assign(row,{ESTADO:'Finalizada',FECHA_FIN:now,VALIDACION_FIN:'CORRECCION_ADMINISTRATIVA',CIERRE_TIPO:'CORRECCION_ADMINISTRATIVA',CIERRE_FUERA_BASE:'NO',CIERRE_MOTIVO:reason,CIERRE_AUTORIZADO_POR:user.ID,CIERRE_AUTORIZADO_ROL:user.ROL_ID,CIERRE_IP_PUBLICA:String(data.IP_PUBLICA||''),CIERRE_FECHA_AUTORIZACION:now,OBSERVACIONES:[row.OBSERVACIONES,`Cierre correctivo administrativo: ${reason}`].filter(Boolean).join(' | '),ACTUALIZADO_EN:now});
    const stillOpenDriver=activeRows(localDb.operations).some(op=>op.ID!==row.ID&&op.CONDUCTOR_ID===row.CONDUCTOR_ID&&op.ESTADO==='Activa'&&!op.FECHA_FIN)||activeRows(localDb.routes).some(route=>route.CONDUCTOR_ID===row.CONDUCTOR_ID&&route.ESTADO==='En curso'&&!route.FECHA_FIN);
    const stillOpenVehicle=activeRows(localDb.operations).some(op=>op.ID!==row.ID&&op.VEHICULO_ID===row.VEHICULO_ID&&op.ESTADO==='Activa'&&!op.FECHA_FIN)||activeRows(localDb.routes).some(route=>route.VEHICULO_ID===row.VEHICULO_ID&&route.ESTADO==='En curso'&&!route.FECHA_FIN);
    const driver=find('drivers',row.CONDUCTOR_ID),vehicle=find('vehicles',row.VEHICULO_ID);if(driver&&!stillOpenDriver)driver.ESTADO='Disponible';if(vehicle&&!stillOpenVehicle)vehicle.ESTADO='Disponible';
    localDb.history.push({ID:id('HIS'),OPERACION_ID:row.ID,EVENTO:'CIERRE_CORRECTIVO_ADMIN',DETALLE:reason,FECHA_HORA:now,USUARIO_ID:user.ID,CREADO_EN:now,ELIMINADO:'NO'});audit(user,'CIERRE_CORRECTIVO_ADMIN','OPERACIONES',reason,row.ID);saveLocal();return{row:cleanRow(row),correccionAdministrativa:true,auditoriaRegistrada:true};
  }
  function localEditOperationAdmin(payload){
    ...
    const before=localOperationSnapshot(row),vehicleId=String(data.VEHICULO_ID||row.VEHICULO_ID||''),driverId=String(data.CONDUCTOR_ID||row.CONDUCTOR_ID||''),routeId=String(data.RUTA_ID??row.RUTA_ID??''),vehicle=find('vehicles',vehicleId),driver=find('drivers',driverId),active=row.ESTADO==='Activa'&&!row.FECHA_FIN;if(!vehicle)throw new Error('VEHICULO_NO_ENCONTRADO');if(!driver)throw new Error('CONDUCTOR_NO_ENCONTRADO');
    if(active&&vehicleId!==row.VEHICULO_ID&&vehicle.ESTADO!=='Disponible')throw new Error('VEHICULO_NO_DISPONIBLE');if(active&&driverId!==row.CONDUCTOR_ID&&driver.ESTADO!=='Disponible')throw new Error('CONDUCTOR_NO_DISPONIBLE');let route=null;if(routeId){route=find('routes',routeId);if(!route)throw new Error('RUTA_NO_ENCONTRADA');if(route.OPERACION_ID&&route.OPERACION_ID!==row.ID&&find('operations',route.OPERACION_ID)?.ESTADO==='Activa'&&!find('operations',route.OPERACION_ID)?.FECHA_FIN)throw new Error('RUTA_YA_VINCULADA');if(route.VEHICULO_ID&&route.VEHICULO_ID!==vehicleId)throw new Error('RUTA_NO_COINCIDE_VEHICULO');if(route.CONDUCTOR_ID&&route.CONDUCTOR_ID!==driverId)throw new Error('RUTA_NO_COINCIDE_CONDUCTOR');}
  ...
  function localDeleteOperationAdmin(payload){
    const user=requireLocalUser();localRequireOperationAdmin(user);const data=payload.data||payload,row=find('operations',payload.id||payload.OPERACION_ID||data.OPERACION_ID);if(!row)throw new Error('REGISTRO_NO_ENCONTRADO');const reason=String(data.MOTIVO_ELIMINACION||'').trim()||'Eliminación administrativa solicitada por el Administrador.',snapshot=localOperationSnapshot(row),active=row.ESTADO==='Activa'&&!row.FECHA_FIN,vehicle=find('vehicles',row.VEHICULO_ID),driver=find('drivers',row.CONDUCTOR_ID);if(row.RUTA_ID){const route=find('routes',row.RUTA_ID);if(route&&route.OPERACION_ID===row.ID)Object.assign(route,active?{OPERACION_ID:'',ESTADO:'Asignada',FECHA_INICIO:'',ACTUALIZADO_EN:iso()}:{OPERACION_ID:'',ACTUALIZADO_EN:iso()});}const detail=`Operación eliminada lógicamente por Administrador. Motivo: ${reason}. Datos: ${JSON.stringify(snapshot)}`;localDb.history.push({ID:id('HIS'),OPERACION_ID:row.ID,EVENTO:'ELIMINACION_ADMIN',DETALLE:detail,FECHA_HORA:iso(),USUARIO_ID:user.ID,CREADO_EN:iso(),ELIMINADO:'NO'});row.ELIMINADO='SI';row.ACTUALIZADO_EN=iso();if(vehicle)localReconcileAvailability('vehicles',vehicle);if(driver)localReconcileAvailability('drivers',driver);audit(user,'ELIMINAR_ADMIN','OPERACIONES',detail,row.ID);saveLocal();return{id:row.ID,eliminacionLogica:true,auditoriaRegistrada:true};
  }
  ...
  function localValidateVehicleQr(payload){
    ...
    if(context==='combustible'){
      ...
      if(user.ROL_ID!=='ROL-ADMIN'){const own=user.ROL_ID==='ROL-CONDUCTOR'?localDriver(user):null;if(user.ROL_ID==='ROL-CONDUCTOR'&&!own)throw new Error('CONDUCTOR_NO_ASOCIADO');const hasOperation=activeRows(localDb.operations).some(row=>row.VEHICULO_ID===vehicle.ID&&row.ESTADO==='Activa'&&!row.FECHA_FIN&&(!own||row.CONDUCTOR_ID===own.ID)),hasRoute=activeRows(localDb.routes).some(row=>row.VEHICULO_ID===vehicle.ID&&['Asignada','En curso'].includes(row.ESTADO)&&!row.FECHA_FIN&&(!own||row.CONDUCTOR_ID===own.ID));if(!hasOperation&&!hasRoute)throw new Error('COMBUSTIBLE_ASIGNACION_ACTIVA_REQUERIDA');}
  ...
  function localSaveLocation(payload) {
    ...
    let operationId=data.OPERACION_ID||'',vehicleId=data.VEHICULO_ID||'';const active=activeRows(localDb.operations).find(x=>x.CONDUCTOR_ID===driverId&&x.ESTADO==='Activa'&&!x.FECHA_FIN);if(active){operationId=operationId||active.ID;vehicleId=vehicleId||active.VEHICULO_ID;}
  ...
  function localRecordRouteHistory(route,event,user,detail=''){const now=iso();localDb.routeHistory.push({ID:id('HRU'),RUTA_ID:route.ID,EVENTO:event,ESTADO:route.ESTADO||'',FECHA_EVENTO:now,CRONOMETRO_INICIO:route.CRONOMETRO_INICIO||route.FECHA_ASIGNACION||'',CRONOMETRO_FIN:route.CRONOMETRO_FIN||route.FECHA_FIN||'',TIEMPO_TRANSCURRIDO_SEGUNDOS:localRouteSeconds(route),TIEMPO_TRANSCURRIDO_TEXTO:route.TIEMPO_TRANSCURRIDO_TEXTO||localFormatElapsed(localRouteSeconds(route)),USUARIO_ID:user.ID,USUARIO_NOMBRE:user.NOMBRE||user.CORREO||'',DETALLE:detail,CREADO_EN:now,ACTUALIZADO_EN:now,ELIMINADO:'NO'});}
  ...
  function localUpdateRouteStatus(payload){
    ...
    const now=iso();
    if(state==='En curso'){
      const driverId=route.CONDUCTOR_ID,operation=activeRows(localDb.operations).find(row=>row.CONDUCTOR_ID===driverId&&row.ESTADO==='Activa'&&!row.FECHA_FIN&&(!route.VEHICULO_ID||row.VEHICULO_ID===route.VEHICULO_ID));
      ...
      route.CRONOMETRO_INICIO=route.CRONOMETRO_INICIO||route.FECHA_ASIGNACION||now;const routeDriver=find('drivers',driverId),routeVehicle=find('vehicles',vehicleId);if(routeDriver)localReconcileAvailability('drivers',routeDriver);if(routeVehicle)localReconcileAvailability('vehicles',routeVehicle);localRecordRouteHistory(route,'INICIADA',user,`GPS activado · check-in ${checkin.ID}`);audit(user,'INICIAR_RUTA','RUTAS',`GPS activado · check-in ${checkin.ID}`,route.ID);saveLocal();return{row:route,seguimiento:{activo:true,RUTA_ID:route.ID,OPERACION_ID:route.OPERACION_ID||'',VEHICULO_ID:vehicleId,CONDUCTOR_ID:driverId,CHECKIN_ID:checkin.ID},operacionVinculada:Boolean(operation)};
    }
    route.ESTADO=state;if(['Completada','Cancelada'].includes(state)){route.FECHA_FIN=now;route.CRONOMETRO_INICIO=route.CRONOMETRO_INICIO||route.FECHA_ASIGNACION||route.CREADO_EN||now;route.CRONOMETRO_FIN=now;route.TIEMPO_TRANSCURRIDO_SEGUNDOS=localRouteSeconds(route,now);route.TIEMPO_TRANSCURRIDO_TEXTO=localFormatElapsed(route.TIEMPO_TRANSCURRIDO_SEGUNDOS);route.GPS_SEGUIMIENTO_ACTIVO='NO';localRecordRouteHistory(route,state==='Completada'?'COMPLETADA':'CANCELADA',user,`Tiempo total: ${route.TIEMPO_TRANSCURRIDO_TEXTO}`);}route.ACTUALIZADO_EN=now;audit(user,state==='Completada'?'COMPLETAR_RUTA':'CAMBIAR_ESTADO','RUTAS',`Estado: ${state}${route.TIEMPO_TRANSCURRIDO_TEXTO?` · ${route.TIEMPO_TRANSCURRIDO_TEXTO}`:''}`,route.ID);
    const stateDriver=find('drivers',route.CONDUCTOR_ID),stateVehicle=find('vehicles',route.VEHICULO_ID);if(stateDriver)localReconcileAvailability('drivers',stateDriver);if(stateVehicle)localReconcileAvailability('vehicles',stateVehicle);if(state==='Completada'&&previousState!=='Completada'){const driver=stateDriver||{},vehicle=stateVehicle||{};localNotifyRoles(['ROL-ADMIN'],{TITULO:`Ruta finalizada: ${route.NOMBRE||route.ID}`,MENSAJE:`Conductor: ${driver.NOMBRE||route.CONDUCTOR_ID}. Vehículo: ${vehicle.PATENTE||route.VEHICULO_ID}.`,TIPO:'Ruta finalizada',PRIORIDAD:'Alta',RUTA_ID:route.ID,OPERACION_ID:route.OPERACION_ID||'',CREADO_POR:user.ID});}
  ...
  function localHeartbeat(payload){
    ...
    const operation=driver?activeRows(localDb.operations).find(row=>row.CONDUCTOR_ID===driver.ID&&row.ESTADO==='Activa'&&!row.FECHA_FIN):null;
    const route=driver?(activeRows(localDb.routes).find(row=>row.CONDUCTOR_ID===driver.ID&&row.ESTADO==='En curso'&&!row.FECHA_FIN)||activeRows(localDb.routes).find(row=>row.CONDUCTOR_ID===driver.ID&&row.ESTADO==='Asignada'&&!row.FECHA_FIN)):null;
  ...
  function localRealtimeSummary(payload={}){
    ...
    const routes=onlyGps?[]:(hasLocalPermission(user,'RUTAS','LEER')?localFilterRows('routes',activeRows(localDb.routes),user):[]).filter(row=>['Asignada','En curso'].includes(row.ESTADO)&&!row.FECHA_FIN);
  ...
  function localBulkImport(payload){
    ...
    if(!['ROL-ADMIN','ROL-SUPERVISOR'].includes(String(user.ROL_ID||'').toUpperCase()))throw new Error('PERMISO_DENEGADO');
