(function () {
  ...
  function localReviewVehicleCheckin(payload){
    const user=requireLocalUser(),data=payload.data||payload,idValue=payload.id||data.CHECKIN_ID,row=find('checkins',idValue);if(!row)throw new Error('CHECKIN_NO_ENCONTRADO');if(row.UTILIZADO==='SI')throw new Error('CHECKIN_YA_UTILIZADO');
  ...
  function localAssignRoute(payload){
    ...
    const checkinId=String(data.CHECKIN_ID||'').trim();const checkin=checkinId?activeRows(localDb.checkins).find(item=>String(item.ID)===checkinId&&item.CONDUCTOR_ID===driver.ID&&item.VEHICULO_ID===vehicle?.ID&&item.ESTADO_REVISION==='Aprobado'&&item.UTILIZADO!=='SI'&&new Date(item.VIGENTE_HASTA||0)>new Date()):activeRows(localDb.checkins).filter(item=>item.CONDUCTOR_ID===driver.ID&&item.VEHICULO_ID===vehicle?.ID&&item.ESTADO_REVISION==='Aprobado'&&item.UTILIZADO!=='SI'&&new Date(item.VIGENTE_HASTA||0)>new Date()).sort((a,b)=>new Date(b.FECHA_HORA)-new Date(a.FECHA_HORA))[0];if(!checkin)throw new Error(checkinId?'CHECKIN_NO_COINCIDE_ASIGNACION':'CHECKIN_APROBADO_REQUERIDO');
  ...
  function localPendingNotices(){
    ...
    const notifications=dedupe(activeRows(localDb.notifications).filter(row=>String(row.LEIDA||'NO').toUpperCase()!=='SI').filter(row=>{const uid=String(row.DESTINATARIO_USUARIO_ID||'').trim(),did=String(row.DESTINATARIO_CONDUCTOR_ID||'').trim();return (!uid&&!did)||uid===String(user.ID)||Boolean(driver&&did===String(driver.ID));}),'N');
    const alerts=dedupe(activeRows(localDb.alerts).filter(row=>String(row.LEIDA||'NO').toUpperCase()!=='SI').filter(row=>{const uid=String(row.USUARIO_ID||'').trim();return !uid||uid===String(user.ID); }),'A');
    return {notifications,alerts,notificaciones:notifications,alertas:alerts,unreadNotifications:notifications.length,unreadAlerts:alerts.length,total:notifications.length+alerts.length};
  }
