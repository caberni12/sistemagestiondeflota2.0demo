(function () {
  ...
  function localUpdateRouteStatus(payload){
    ...
    const now=iso();
    if(state==='Completada'){
      let stops=localParseRouteStops(route.PARADAS_CODIFICADAS);if(stops.length>=2){const pending=stops.filter(stop=>stop.ESTADO!=='COMPLETADA'),last=stops[stops.length-1];if(pending.length===1&&pending[0].PUNTO_ID===last.PUNTO_ID&&pending[0].ESTADO==='LLEGADA_DETECTADA'){last.ESTADO='COMPLETADA';last.COMPLETADA_EN=now;route.PARADAS_CODIFICADAS=JSON.stringify(stops);}else if(pending.length)throw new Error('RUTA_MULTIPLE_DESTINOS_PENDIENTES');}
    }
    if(state==='En curso'){
  ...
  function localNexoText(value){return String(value??'').trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');}
